...
3. **Semantic classification** — a lightweight ML model (~2 MB, ONNX) classifies each audio segment into one of four states:
...
If the ONNX model fails to load (e.g., no WASM support), Utterance falls back to an **EnergyVAD** baseline classifier. It uses RMS energy thresholds to detect speech vs. silence and relies on pause duration to infer turn completion.

The baseline is functional but cannot distinguish thinking pauses from turn completion with the same accuracy as the ML model.
